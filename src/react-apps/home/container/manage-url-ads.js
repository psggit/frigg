...
import ViewUrlAds from '../components/manage-image-ads/view-ads'
...
class ManageUrlAds extends React.Component {
  constructor(props) {
    super(props)
    this.filter = {
      stateShortName: null,
  ...
  fetchData() {
    ...
    if (location.search.length) {
      ...
      this.props.actions.fetchUrlAds({
        city_id: null,
        offset: 0,
        limit: 10
      })
  ...
  setPage(pageObj) {
    ...
    history.pushState(queryObj, "city listing", `/home/manage-url-ads?${getQueryUri(queryObj)}`)
  }
  ...
  render() {
    ...
    return (
      <div style={{ width: '100%' }}>
        ...
        >
          <div>
            <NavLink to={`${location.pathname}/create-new-url-ad`}>
              <RaisedButton
                label="Create new ad"
                primary
        ...
        {
          !loadingUrlAds && urlAdsData.ads_data.length && this.state.cityName
        ...
        <ViewUrlAds
          urlAdsData={urlAdsData.ads_data}
          loadingUrlAds={loadingUrlAds}
          updateUrlAdStatus={this.props.actions.updateUrlAdStatus}
          updateUrlAdStatusCB={this.fetchData}
        />

        {
          !loadingUrlAds && urlAdsData.ads_data.length
          ? <Pagination
            activePage={parseInt(this.state.activePage)}
            itemsCountPerPage={10}
            totalItemsCount={urlAdsData.count}
